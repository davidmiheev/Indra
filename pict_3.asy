...
mob_matrix mult(mob_matrix A, mob_matrix B) { 
    return(mob_matrix(A.a * B.a + A.b * B.c, 
                    A.a * B.b + A.b * B.d, 
                    A.c * B.a + A.d * B.c, 
                    A.c * B.b + A.d * B.d));
}
...
void constrWords(int[] m,int level, int mlen, mob_matrix[] mm, pen[] colors, circle[] circ,int j) {
     ...
     for(k = 1; k < 5; ++k) { 
	     ...
	     if(i == (k%4))  { 
	         m.push((int)k%4);
             for(i=j;i<j+3;++i) { 
                 circ[i%4] = TxCirc(mm[(k-1)%4],circ[i%4]); 
                 if(circ[i%4].r > (real)10^(-3))  
                     draw(shift(circ[i%4].C) * scale(circ[i%4].r) * unitcircle, 
                         colors[level % colors.length] + (real)10^(-level)); 
        } 
    ...
    i = m.pop();
    for(k = j;k < j+3; ++k) { 
        if(i%2 == 1) circ[k%4] = TxCirc(mm[i%4],circ[k%4]); 
        else circ[k%4] = TxCirc(mm[(i+2)%4],circ[k%4]); 
    }
}
...
for(i=1; i<5; ++i) { 
    ...
    circ.push(C_b); 
    circ.push(C_B); 
    constrWords(m,1,N_ITER,mm,colors,circ,(int)i%4); 
    circ.pop(); 
    circ.pop(); 
    circ.pop(); 
    circ.pop();  
